...
“Wh{wr #3”. O resultado do segundo processamento inverte os caracteres e produz “3# rw{hW”.
...
N (1 ≤ N ≤ 1*104), indicando a quantidade de linhas que o problema deve tratar.
...
public class Beecrowd1024 {
    ...
    public static void main(String[] args) {
        ...
        for(int i = 0; i < N; i++){
            ...
            StringBuilder linha = new StringBuilder(scanner.nextLine());

            // PRIMEIRA PASSADA
            for(int j = 0; j < linha.length(); j++){
                if(Character.isAlphabetic(linha.charAt(j))){
                    linha.setCharAt(j, (char) (linha.charAt(j) + 3));
                }
            }
            ...
            //TERCEIRA PASSADA
            for(int k = linha.length() / 2; k < linha.length(); k++){
                linha.setCharAt(k, (char) (linha.charAt(k) - 1));
            }
